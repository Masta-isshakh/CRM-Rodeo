...
export default function MainLayout({ signOut }: { signOut: () => void }) {
  ...
  const [page, setPage] = useState<Page>(() => {
    try {
      const saved = window.sessionStorage.getItem("crm.activePage");
  ...
  const go = (p: Page) => {
    setPage(p);
    try { window.sessionStorage.setItem("crm.activePage", p); } catch { /* ignore */ }
  ...
  const handleModuleNavigate = useCallback(
    (moduleName: string, payload?: any) => {
      ...
      setPage(targetPage);
      try { window.sessionStorage.setItem("crm.activePage", targetPage); } catch { /* ignore */ }
      if (!isDesktop) setSidebarOpen(false);
    },
    [isDesktop]
  );
    ...
    <ApprovalRequestsProvider>
      <div className={`layout-root ${isDesktop ? "desktop-sidebar" : "mobile-sidebar"} ${themeMode === "dark" ? "theme-dark" : "theme-light"}`}>
        ...
        <aside className={`drawer ${isDesktop || sidebarOpen ? "open" : ""}`} aria-hidden={!isDesktop && !sidebarOpen}>
          ...
          <div className="drawer-focus-card" role="note" aria-label={t("Today at a glance")}> 
            <div className="drawer-focus-title"><i className="fas fa-calendar-check" aria-hidden="true" /> {t("Today at a glance")}</div>
          ...
          <nav className="drawer-nav">
            ...
            {show.dashboard && (
              <button className={page === "dashboard" ? "active" : ""} onClick={() => go("dashboard")}>
                <i className="fas fa-table-cells-large" aria-hidden="true" /> {t("Dashboard")}
              </button>
            )}

            {show.dailyreport && (
              <button className={page === "dailyreport" ? "active" : ""} onClick={() => go("dailyreport")}>
                <i className="fas fa-chart-column" aria-hidden="true" /> {t("Daily Report")}
              </button>
            )}

            {show.activitylog && (
              <button className={page === "activitylog" ? "active" : ""} onClick={() => go("activitylog")}>
                <i className="fas fa-clock" aria-hidden="true" /> {t("Activity Log")}
              </button>
            )}
            ...
            {show.filesharing && (
              <button className={page === "filesharing" ? "active" : ""} onClick={() => go("filesharing")}>
                <i className="fas fa-folder-open" aria-hidden="true" /> {t("File Sharing")}
              </button>
            )}

              {show.pushnotifications && (
                <button className={page === "pushnotifications" ? "active" : ""} onClick={() => go("pushnotifications")}>
                  <i className="fas fa-bell" aria-hidden="true" /> {t("Push Notifications")}
                </button>
              )}
            ...
            {show.tickets && (
              <button className={page === "tickets" ? "active" : ""} onClick={() => go("tickets")}>
                <i className="fas fa-ticket" aria-hidden="true" /> {t("Tickets")}
              </button>
            )}
              ...
              <div className="drawer-section">
                ...
                {showAdmin.users && (
                  <button className={page === "users" ? "active" : ""} onClick={() => go("users")}>
                    <i className="fas fa-users" aria-hidden="true" /> {t("Users")}
                  </button>
                )}
                {showAdmin.departments && (
                  <button className={page === "departments" ? "active" : ""} onClick={() => go("departments")}>
                    <i className="fas fa-building" aria-hidden="true" /> {t("Departments")}
                  </button>
                )}
                {showAdmin.rolespolicies && (
                  <button className={page === "rolespolicies" ? "active" : ""} onClick={() => go("rolespolicies")}>
                    <i className="fas fa-user-shield" aria-hidden="true" /> {t("Roles & Policies")}
                  </button>
                )}
                {showAdmin.campaignaudience && (
                ...
                {showAdmin.dbcleanup && (
                  <button className={page === "dbcleanup" ? "active" : ""} onClick={() => go("dbcleanup")}>
                    <i className="fas fa-database" aria-hidden="true" /> {t("Database Cleanup")}
                  </button>
                )}
        ...
        <div className="layout-main">
          ...
          <main className="content">
            ...
            {page === "filesharing" && show.filesharing && (
              <PermissionGate moduleId="filesharing" optionId="filesharing_list">
                <LocalPageErrorBoundary
          ...
          <footer className="layout-footer" role="contentinfo">
            <div className="layout-footer-left">
              <span className="layout-footer-dot" aria-hidden="true" />
              <span>{t("CRM Rodeo Premium Workspace")}</span>
            </div>
            <div className="layout-footer-right">{t("All rights reserved")} • {currentYear}</div>
          </footer>
        </div>
      </div>
    </ApprovalRequestsProvider>
