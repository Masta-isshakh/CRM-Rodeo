...
export default function VoucherGiftPage({ currentUser }: { currentUser?: any; permissions?: any }) {
  ...
  const openVoucherHistoryEntry = async (row: VoucherHistoryRow) => {
    try {
      const blob = await buildVoucherHistoryPdfBlob(row);
      ...
      console.error("Failed to open voucher from history", error);
      setStatus(t("Failed to open voucher from history."));
    }
  };

  const downloadVoucherHistoryEntry = async (row: VoucherHistoryRow) => {
    try {
      const blob = await buildVoucherHistoryPdfBlob(row);
      const url = URL.createObjectURL(blob);
      const link = document.createElement("a");
      link.href = url;
      link.download = `${row.quoteNumber || "voucher-history"}.pdf`;
      document.body.appendChild(link);
      link.click();
      link.remove();
      window.setTimeout(() => URL.revokeObjectURL(url), 3000);
    } catch (error) {
      console.error("Failed to download voucher from history", error);
      setStatus(t("Failed to download voucher from history."));
    }
  };

  const deleteVoucherHistoryEntry = async (row: VoucherHistoryRow) => {
    const confirmed = window.confirm(t("Delete this voucher history entry?"));
    if (!confirmed) return;

    try {
      await client.models.VoucherGiftHistory.delete({ id: row.id } as any);
      setVoucherHistoryRows((prev) => prev.filter((item) => item.id !== row.id));
      setStatus(t("Voucher history entry deleted."));
    } catch (error) {
      console.error("Failed to delete voucher history", error);
      setStatus(t("Failed to delete voucher history entry."));
    }
  };
                  ...
                  <tbody>
                    {voucherHistoryRows.map((row) => {
                      ...
                      return (
                        <tr
                          key={row.id}
                          onClick={() => void openVoucherHistoryEntry(row)}
                          style={{ cursor: "pointer" }}
                          title={t("Click to open voucher")}
                        >
                          ...
                          <td style={{ borderBottom: "1px solid #eef2f7", padding: "8px", textAlign: "right" }}>
                            <div style={{ display: "inline-flex", gap: 8 }}>
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  void downloadVoucherHistoryEntry(row);
                                }}
                              >
                              ...
                              <button
                                ...
                                onClick={(e) => {
                                  e.stopPropagation();
                                  void deleteVoucherHistoryEntry(row);
                                }}
                              >
