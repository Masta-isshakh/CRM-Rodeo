...
export async function getJobOrderByOrderNumber(orderKey: string): Promise<any | null> {
  ...
  const services = parsedServices.map((s: any, idx: number) => {
    ...
    return {
      ...
      specificationBrandId: String(s.specificationBrandId ?? "").trim() || null,
      specificationBrandName: String(s.specificationBrandName ?? "").trim() || null,
      specificationColorHex: String(s.specificationColorHex ?? "").trim() || null,
      specificationProductId: String(s.specificationProductId ?? "").trim() || null,
      specificationProductName: String(s.specificationProductName ?? "").trim() || null,
      specificationMeasurement: String(s.specificationMeasurement ?? "").trim() || null,
