...
import { CognitoIdentityProviderClient, ListGroupsCommand, CreateGroupCommand, DeleteGroupCommand, ListUsersCommand, AdminAddUserToGroupCommand, AdminRemoveUserFromGroupCommand, AdminListGroupsForUserCommand, ListUsersInGroupCommand, } from "@aws-sdk/client-cognito-identity-provider";
...
async function listAllGroups() {
    ...
    do {
        ...
        for (const g of res.Groups ?? []) {
            ...
            if (!groupName.startsWith(DEPT_PREFIX))
                continue;
            out.push({
                key: groupName,
...
async function createDepartment(departmentName) {
    ...
    if (!clean)
        throw new Error("Department name required.");
...
async function renameDepartment(oldKey, newName) {
    const oldK = oldKey.trim();
    const newN = newName.trim();
    if (!oldK || !newN)
        throw new Error("Old key + new name required.");
...
async function deleteDepartment(key) {
    const k = key.trim();
    if (!k)
        throw new Error("Department key required.");
    // Safety: block delete if it contains users
    const usersRes = await cognito.send(new ListUsersInGroupCommand({
        UserPoolId: USERPOOL_ID,
        GroupName: k,
        Limit: 1,
    }));
...
async function addUserToDepartment(username, departmentKey) {
    if (!username?.trim())
...
async function removeUserFromDepartment(username, departmentKey) {
    if (!username?.trim())
        throw new Error("Username required.");
    if (!departmentKey?.trim())
        throw new Error("Department key required.");
...
async function listUserDepartments(username) {
    if (!username?.trim())
        throw new Error("Username required.");
    ...
    const departments = (res.Groups ?? [])
        .map((g) => g.GroupName)
        .filter((x) => !!x && x.startsWith(DEPT_PREFIX));
    return { departments };
}
