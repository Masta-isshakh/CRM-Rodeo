import { AdminDisableUserCommand, AdminEnableUserCommand, AdminGetUserCommand, AdminListGroupsForUserCommand, CognitoIdentityProviderClient, ListUsersCommand, } from "@aws-sdk/client-cognito-identity-provider";
...
function actorUsernameFromEvent(event) {
    const claims = event?.identity?.claims ?? event?.request?.userAttributes ?? {};
    return normalizeKey(claims?.["cognito:username"] ?? claims?.username ?? claims?.email ?? "");
}
...
async function canEditUsers(dataClient, event, userPoolId) {
    ...
    try {
        ...
        const roleToggles = (allToggles?.data ?? []).filter((t) => roleIds.includes(t?.roleId));
...
export const handler = async (event) => {
    ...
    if (profile?.id) {
        await dataClient.models.UserProfile.update({
            ...
            isActive: !!isActive,
            dashboardAccessEnabled: isActive ? Boolean(profile.dashboardAccessEnabled ?? true) : false,
            ...
            lineManagerEmail: profile.lineManagerEmail ?? undefined,
            lineManagerName: profile.lineManagerName ?? undefined,
            failedLoginAttempts: Number(profile.failedLoginAttempts ?? 0),
            lastFailedLoginAt: profile.lastFailedLoginAt ?? undefined,
