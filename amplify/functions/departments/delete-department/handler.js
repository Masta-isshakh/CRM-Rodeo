import { CognitoIdentityProviderClient, DeleteGroupCommand, ListUsersInGroupCommand, } from "@aws-sdk/client-cognito-identity-provider";
...
export const handler = async (event) => {
    ...
    if (!allowed)
        throw new Error("Not authorized to delete departments. Check roles and policies configuration.");
    const key = event.arguments.departmentKey.trim();
    if (!key)
        throw new Error("departmentKey is required");
    const users = await cognito.send(new ListUsersInGroupCommand({ UserPoolId: userPoolId, GroupName: key, Limit: 2 }));
