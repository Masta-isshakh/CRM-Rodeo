import { AdminAddUserToGroupCommand, AdminListGroupsForUserCommand, AdminRemoveUserFromGroupCommand, CognitoIdentityProviderClient, CreateGroupCommand, GetGroupCommand, AdminGetUserCommand, ListUsersCommand, } from "@aws-sdk/client-cognito-identity-provider";
...
function actorUsernameFromEvent(event) {
    const claims = event?.identity?.claims ?? event?.request?.userAttributes ?? {};
    return normalizeKey(claims?.["cognito:username"] ?? claims?.username ?? claims?.email ?? event?.identity?.username ?? "");
}
function actorSubFromEvent(event) {
    const claims = event?.identity?.claims ?? event?.request?.userAttributes ?? {};
    return String(claims?.sub ?? "").trim();
}
function expandDepartmentCandidates(dept) {
...
async function resolveGroups(userPoolId, event) {
    const claims = event?.identity?.claims ?? event?.request?.userAttributes ?? {};
...
async function findUserProfileForActor(dataClient, event) {
    ...
    if (email) {
        try {
            ...
            if (row?.id)
                return { profile: row, email, username, sub };
        }
        catch {
            // continue
        }
...
function aggregateToggleMap(toggleRecords) {
    const map = {};
...
function isToggleExplicit(toggleMap, key) {
    return Object.prototype.hasOwnProperty.call(toggleMap, key);
}
async function canEditUsers(dataClient, event, userPoolId) {
    ...
    try {
        ...
        if (actorRoleId) {
            ...
            for (const candidate of candidates) {
                ...
                for (const link of deptLinks?.data ?? []) {
                    const rid = String(link?.roleId ?? "").trim();
                    if (rid)
                        collected.add(rid);
                }
            }
        ...
        const roleToggles = (allToggles?.data ?? []).filter((t) => roleIdSet.has(String(t?.roleId ?? "").trim()));
        ...
        if (!moduleEnabled)
            return false;
        if (editAllowedByOption !== null)
            return Boolean(editAllowedByOption);
        const policies = await dataClient.models.RolePolicy.list({ limit: 30000 });
        let canUpdate = false;
        for (const p of policies?.data ?? []) {
            const rid = String(p?.roleId ?? "").trim();
            if (!roleIdSet.has(rid))
                continue;
            const key = normalizeKey(p?.policyKey ?? "");
            if (key !== "users_admin")
                continue;
            canUpdate = canUpdate || Boolean(p?.canUpdate);
            if (canUpdate)
                break;
        }
...
export const handler = async (event) => {
    ...
    if (!userPoolId)
        throw new Error("Missing Cognito User Pool env var (AMPLIFY_AUTH_USERPOOL_ID/USERPOOL_ID/USER_POOL_ID)");
    ...
    const groupsRes = await cognito.send(new AdminListGroupsForUserCommand({ UserPoolId: userPoolId, Username: username }));
    const current = (groupsRes.Groups ?? []).map((g) => g.GroupName).filter(Boolean);
    ...
    if (!profile?.id)
        throw new Error(`UserProfile not found for ${email}`);
