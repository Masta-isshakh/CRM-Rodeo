 ...
 */
function toE164(raw, defaultCC = DEFAULT_CC) {
    const cleaned = raw.replace(/[\s\-().]/g, "");
    if (!cleaned)
        return null;
    if (/^\+\d{7,15}$/.test(cleaned))
        return cleaned; // already E.164
    if (/^00\d{7,14}$/.test(cleaned))
        return `+${cleaned.slice(2)}`; // 00 prefix
...
export const handler = async (event) => {
    ...
    const rawPhones = Array.isArray(args.phones) ? args.phones : [];
    const message = String(args.message ?? "").trim();
    const smsType = String(args.smsType ?? "Transactional"); // Transactional | Promotional
    const batchId = String(args.batchId ?? "").trim();
    const smsLogId = String(args.smsLogId ?? "").trim();
    if (!message)
        return { ok: false, error: "Message is required.", results: [] };
    if (!rawPhones.length)
        return { ok: false, error: "No phone numbers provided.", results: [] };
    const results = [];
    for (const raw of rawPhones) {
        const normalised = toE164(String(raw ?? "").trim());
        const sentAt = new Date().toISOString();
        const publishAuditEvent = async (payload) => {
            if (!AUDIT_TOPIC_ARN)
                return;
