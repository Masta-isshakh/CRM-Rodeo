...
function response(statusCode: number, payload: Record<string, unknown>): Response {
  return {
    statusCode,
    headers: {
...
export const handler = async (event: Event): Promise<Response> => {
  ...
  try {
    ...
    const linksRes = await (client.models as any).DriveShareLink.list({
      filter: { token: { eq: token } },
      limit: 1,
    });

    const link = (linksRes?.data ?? [])[0] as any;
    if (!link?.id) {
    ...
    const fileRes = await (client.models as any).FileShareItem.get({ id: String(link.fileShareItemId ?? "") });
    const file = (fileRes?.data ?? fileRes) as any;
