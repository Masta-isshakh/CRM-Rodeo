import { CognitoIdentityProviderClient, AdminAddUserToGroupCommand, AdminCreateUserCommand, AdminDeleteUserCommand, AdminGetUserCommand, AdminResetUserPasswordCommand, AdminUpdateUserAttributesCommand, AdminListGroupsForUserCommand, GetGroupCommand, CreateGroupCommand, ListUsersCommand, } from "@aws-sdk/client-cognito-identity-provider";
...
async function findUserProfileByEmailCaseInsensitive(dataClient, email) {
...
async function canInviteUsers(dataClient, event, userPoolId) {
    ...
    if (!departmentKey && !actorRoleId)
        return false;
    const fetchLinksForDept = async (dk) => await listAll((args) => dataClient.models.DepartmentRoleLink.list({
        ...args,
    ...
    const roleIds = actorRoleId
        ? [actorRoleId]
        : Array.from(new Set((links ?? []).map((l) => String(l?.roleId ?? "").trim()).filter(Boolean)));
    ...
    for (const policy of allPolicies ?? []) {
        ...
        if (key !== "USERS_ADMIN")
            continue;
        canCreate = canCreate || Boolean(policy?.canCreate);
        canUpdate = canUpdate || Boolean(policy?.canUpdate);
...
async function listAllCognitoUsers(userPoolId, max = 5000) {
    const out = [];
    let paginationToken;
    while (out.length < max) {
...
async function resolveCognitoUsernames(userPoolId, email, sub) {
...
async function ensureGroup(userPoolId, groupName, description) {
    try {
...
async function resolveCognitoUsername(userPoolId, email) {
    try {
...
export const handler = async (event) => {
    const employeeId = String(event.arguments?.employeeId ?? "").trim();
    const lineManagerEmail = String(event.arguments?.lineManagerEmail ?? "").trim().toLowerCase();
    const lineManagerName = String(event.arguments?.lineManagerName ?? "").trim();
    ...
    const roleId = String(event.arguments?.roleId ?? "").trim();
    // ✅ NEW (backward-compatible)
    const mobileNumberRaw = event.arguments?.mobileNumber;
    ...
    if (!mobileNumber) {
        console.warn("invite-user: mobileNumber missing from event.arguments. Update amplify/data/resource.ts mutation args + regenerate client.");
    }
    ...
    try {
        ...
        if (!existingProfile?.id && staleUsernames.length) {
            ...
            const recreated = await cognito.send(new AdminCreateUserCommand({
                UserPoolId: userPoolId,
                Username: email,
                TemporaryPassword: temporaryPassword,
                UserAttributes: [
            ...
            try {
                ...
                const isCurrentStateResetIssue = resetErrName.includes("invalidparameter") &&
    ...
    if (!sub)
        throw new Error("Could not resolve user sub.");
