import { AdminDeleteUserCommand, AdminGetUserCommand, AdminListGroupsForUserCommand, CognitoIdentityProviderClient, ListUsersCommand, } from "@aws-sdk/client-cognito-identity-provider";
...
async function canDeleteUsers(dataClient, event, userPoolId) {
    ...
    try {
        ...
        if (!moduleEnabled)
            return false;
        if (deleteAllowedByOption !== null)
            return Boolean(deleteAllowedByOption);
        ...
        for (const p of policies?.data ?? []) {
            ...
            if (key !== "users_admin")
                continue;
            canDelete = canDelete || Boolean(p?.canDelete);
            if (canDelete)
                break;
        }
...
async function resolveCognitoUsernames(userPoolId, email, subCandidates = []) {
    const normalizedEmail = String(email ?? "").trim().toLowerCase();
    const normalizedSubs = new Set((subCandidates ?? []).map((value) => String(value ?? "").trim()).filter(Boolean));
...
export const handler = async (event) => {
    ...
    const subCandidates = (profiles.data ?? [])
        .map((profile) => String(profile?.profileOwner ?? "").split("::")[0]?.trim() || "")
        .filter(Boolean);
