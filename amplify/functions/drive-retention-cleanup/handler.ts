...
export const handler = async () => {
  ...
  try {
    ...
    const filesRes = await (client.models as any).FileShareItem.list({ limit: 5000 });
    const files = (filesRes?.data ?? []) as any[];

    for (const row of files) {
      if (!row?.id || !row?.isDeleted) continue;
      const deletedAtMs = Date.parse(String(row.deletedAt ?? ""));
      if (!Number.isFinite(deletedAtMs) || deletedAtMs > trashBefore) continue;
      ...
      try {
        ...
        await (client.models as any).FileShareItem.delete({ id: row.id });
        stats.filesPurged += 1;
      } catch {
        stats.errors += 1;
      }
    ...
    for (const link of links) {
      ...
      try {
        ...
        if (oldRevoked || oldExpired) {
          await (client.models as any).DriveShareLink.delete({ id: link.id });
