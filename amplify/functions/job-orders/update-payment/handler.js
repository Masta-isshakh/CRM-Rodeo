...
export const handler = async (event) => {
    ...
    const next = { id, updatedAt: nowIso() };
    ...
    if (event.arguments?.method != null)
        next.method = String(event.arguments.method ?? "").trim() || "Cash";
    if (event.arguments?.reference != null)
        next.reference = String(event.arguments.reference ?? "").trim() || undefined;
    if (event.arguments?.paidAt != null)
        next.paidAt = String(event.arguments.paidAt ?? "").trim() || row.paidAt;
    if (event.arguments?.notes != null)
        next.notes = String(event.arguments.notes ?? "").trim() || undefined;
