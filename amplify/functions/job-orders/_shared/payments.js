...
async function listAll(listFn, max = 5000) {
    const out = [];
 ...
 */
export async function recomputeJobOrderPaymentSummary(client, jobOrderIdRaw) {
    const jobOrderId = String(jobOrderIdRaw ?? "").trim();
    if (!jobOrderId)
        return;
    // 1) Load JobOrder
    const g = await client.models.JobOrder.get({ id: jobOrderId });
    const job = g?.data ?? g;
    if (!job?.id)
        return;
    ...
    let payments = [];
    try {
        const byIdx = await client.models.JobOrderPayment.listPaymentsByJobOrder?.({
            jobOrderId,
            limit: 1000,
        });
        if (byIdx?.nextToken) {
            payments = await listAll((args) => client.models.JobOrderPayment.listPaymentsByJobOrder({
                jobOrderId,
                ...args,
            }), 5000);
        }
    ...
    const sumPaid = (payments ?? []).reduce((acc, p) => {
        const status = String(p?.paymentStatus ?? "COMPLETED").trim().toUpperCase();
        if (status === "VOID" || status === "CANCELLED" || status === "FAILED")
            return acc;
    ...
    const parsed = safeJsonParse(job.dataJson, {});
    ...
    const latest = [...(payments ?? [])].sort((a, b) => String(b?.paidAt ?? b?.createdAt ?? "").localeCompare(String(a?.paidAt ?? a?.createdAt ?? "")))[0];
    const latestMethod = String(latest?.method ?? "").trim();
    const paymentMethod = latestMethod || String(job.paymentMethod ?? "").trim() || undefined;
