...
function resolveLogoUrl(origin) {
...
function buildUrls(username, email, mode) {
    ...
    const setPasswordUrl = mode === "reset"
...
export const handler = async (event) => {
    ...
    const realUsername = String(event.userName ?? email ?? "").trim();
    const codePlaceholder = event.request.codeParameter; // {####} (temp password or reset code)
    const { signInUrl, setPasswordUrl } = buildUrls(realUsername, email, event.triggerSource === "CustomMessage_AdminCreateUser" ? "first" : "reset");
    ...
    if (event.triggerSource === "CustomMessage_AdminResetUserPassword") {
        if (allowEmailOverride) {
            event.response.emailSubject = "Reset your password";
            event.response.emailMessage = buildResetEmailHtml({
                employeeName,
